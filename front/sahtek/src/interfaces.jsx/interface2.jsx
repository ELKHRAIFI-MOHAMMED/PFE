...
import { afficher_malade } from "../redux/actions";
const Interface2 = () => {
  const formData = useSelector((state) => state.user);
  const { nom, prenom, genre, ville } = formData;
  const navigate = useNavigate();
  const dispatch=useDispatch()
  const [list_malade, setList_malade] = useState("");
  const [list_partie, setList_partie] = useState(useSelector(state=>state.list_malade));
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/api/afficher-partie_corp").then((res) => {
      setList_partie(res.data);
      console.log(res);
    });
  }, []);
  ...
  const afficher = (idp) => {
    ...
    alert(genre);
    axios
      .get(`http://127.0.0.1:8000/api/afficher-malade/${idp}/${genre}`)
      .then((res) => {
        //setList_malade(res.data);
        dispatch(afficher_malade(res.data))
        console.log(res);
      });
  ...
  const consulter=(malade)=>{
    navigate('/interface3', { state: malade })
  }

  return (
    <div>
      {console.log(list_partie)}
      {console.log(formData)}
      {console.log(genre)}
      {formData ? (
        <div>
          {/* {console.log(list_partie)}
  {console.log(list_malade)} */}
          <div class="navbar">
          ...
          {genre === 2 ? (
            <div id="h" style={{ backgroundColor: "cyan", color: "black" }}>
              <strong>
                Bonjour Monsieur {nom} {prenom} <br></br>📍{ville}
              </strong>
            </div>
          ) : genre === 3 ? (
            <div id="h" style={{ backgroundColor: "pink", color: "black" }}>
              <strong>
                Bonjour Madame {nom} {prenom} <br></br>📍{ville}
              </strong>
            </div>
          ) : (
            <div>
              Bonjour {nom} {prenom} <br></br>📍{ville}
            </div>
          )}
          <h2
            style={{
          ...
          >
            {" "}
            Veuillez choisir la zone ou vous avez la douleur :{" "}
          </h2>
          ...
          <div>
            {list_malade !== ""
              ? list_malade.map((i) => <li onClick={()=>{consulter(i)}}>{i.nom}</li>)
              : "hhdhdh"}
          </div>
          ...
          <footer>
            ...
            <p>
              ...
              medical advice. Please consult with a healthcare professional for
